...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        att = att.masked_fill(self.mask[:,:,:T,:T].repeat(1,1,T // self.mask.size(2), T // self.mask.size(3)) == 0, float('-inf'))
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.global_pos_emb = nn.Parameter(torch.zeros(1, config.max_timestep, config.n_embed))
        ...
        logger.info("Number of Transformer Parameters: %e", sum(p.numel() for p in self.parameters()))
        ...
        self.state_encoder = nn.Sequential(nn.Linear(1024+2, config.n_embed), nn.Tanh())
        ...
        self.action_embeddings = nn.Sequential(nn.Linear(config.vocab_size, config.n_embed), nn.Tanh())
    ...
    def forward(self, states, actions, targets=None, rtgs=None, timesteps=None):
        ...
        state_embeddings = self.state_encoder(states) # (batch * block_size, n_embed)
        state_embeddings = state_embeddings.reshape(states.shape[0], states.shape[1], self.config.n_embed) # (batch, block_size, n_embed)
        ...
        # Need to change with consideration of my new input dimensions
